...
const populateDaySelectMenu = () => {
  ...
  availability.data.forEach((day, index) => {
    ...
    option.disabled = day.slots.length === 0 || !day.slots.find((slot) => slot.available > 0);
...
const createTimeslotElement = (timeslot) => {
  ...
  button.innerHTML = `<b>${formatTime(timeslot.start)} - ${formatTime(timeslot.end)}</b>${timeslot.priority ? ' ⚡' : ''}<br> $${(
    timeslot.price / 100
  ).toFixed(2)}`;
